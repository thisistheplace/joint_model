...
def map_to_np(inter: Any):
    match type(inter):
        case type(Point3D):
            return NpPoint3D(array=np.array([inter.x, inter.y, inter.z]))
        case type(Vector3D):
            return NpVector3D(array=np.array([inter.x, inter.y, inter.z]))
        case type(Axis3D):
            return NpAxis3D(
        ...
        case type(Model):
            return NpModel(name=inter.name, joint=interface_to_np(inter.model))
        case _:
            raise TypeError(
                f"Could not match interface type {type(inter)} and numpy types"
            )
